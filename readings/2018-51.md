...
a single man team since 2018. The `Readings ${YEAR}.${WEEK}` series is comprised of
a bunch of interesting posts I found in the Internet and I hope it would please you.
...
It uses less handshakes, improves forward error correction and congestion control, and avoids HoL blocking.
...
This post explore the organizational and cultural benefits of moving to a microservices architecture.
...
From organizational perspective, it leads to smaller (and more maintainable) code repos, clearer boundaries between teams, etc.

One of the goals of microservices architecture is to let development teams achieve better results.

The author thinkgs starting from monolithic and moving to microservices when it makes sense.
...
Service Mesh is getting more mature, while middleware can be ideal for transporting information between monolithic applications. Migrating from Middleware to Service Mesh might need uproot the ways their application and infrastructure.
...
* We amplifies the signals via latencies, etc.
...
* Statemap: a visualization of entity state over time. It operate on a payload of JSON where each line corresponds to a state transition for an entity. For example:
  ```
...
* Because of the tangling modules, the growth of business, and the increasing number of incidents, they decided to migrate to SOA.
...
* Solution:
  ...
  * Store all the data on SSD (RocksDB) and the active/hot data in RAM (Memcached). The problem is the poor performance for large cluster when overwriting existing data on RocksDB (for compactions).
...
* run tool: the entrypoint of the invocation image.
...
The package `storage` adds, fetches, removes tasks, performing on a certain storage. Noop means keeping in-memory.
...
The entry point of `Scheduler` is a loop running forever. It try to run all pending jobs every second. The pending jobs are from the storage selecting those supposed to run in this second.
